                     ...
                     const stabilizerStep_t stabilizerStep)
{
  ...
  if (use_observer) {
    ...
    p_x += dt * v_x;
    p_y += dt * v_y;
    p_z += dt * (v_z - 24.182550030836126f * r_err);
    psi += dt * w_z;
    theta += dt * (w_y - 0.012599720590375068f * n_x_err);
    phi += dt * (w_x + 0.02243168759258665f * n_y_err);
    v_x += dt * (g * theta - 0.21657970921070258f * n_x_err);
    v_y += dt * (-g * phi - 0.2894168179349658f * n_y_err);
    v_z += dt * ((a_z - g) - 113.84082632339036f * r_err);
